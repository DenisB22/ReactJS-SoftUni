...
export const Register = ({ setCards }) => {
  ...
  const handleGenderChange = (event) => {
    ...
    if (!genderValue) {
      ...
      setGenderError("Please enter a valid gender - Male, Female, male, female!");
      setIsGenderValid(false);
    } else {
      setGenderError("");
  ...
  const handleAdditionalInfoChange = (event) => {
    ...
    } else if (additionalInfoValue.length < 2 || additionalInfoValue.length > 200) {
      setAdditionalInfoError("Additional info must be less than 200 characters!");
      setIsAdditionalInfoValid(false);
    } else {
  ...
  }, [   passwordError, firstNameError,    lastNameError,    cityError,    countryError,    genderError,    ageError,    breedError,    imageError,    additionalInfoError,    emailError,  ]);
